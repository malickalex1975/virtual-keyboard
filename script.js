...
const information1 = document.createElement('p');
...
const pressedCapsLock = () => {
  capsLock = !capsLock;
  const btn = document.getElementById('key-29');
  if (capsLock) {
    btn.style.color = '#bbeb11';
  } else btn.style.color = '#ffffff';
  if (capsLock) {
    if (lang === 'ENG') {
      keys = keys5;
    } else keys = keys6;
...
const realKeyDown = (code, e) => {
  ...
  const keyNumber = keyCodes.indexOf(code);
